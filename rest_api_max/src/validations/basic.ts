...
export const basicAuthMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!authHeader || !authHeader.startsWith("Basic ")) {
        return res.status(401).json({ status: "error", message: "Make sure authentication is basic, dummy..." });
    }
    ...
    try {
        ...
        if (!userProfile) {
            return res.status(401).json({ status: "error", message: "Could not find profile, is the username and password correct?" });
        }
